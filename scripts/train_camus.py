...
from data import SemiSegDataset_VT, SemiSegValidDataset,SemiDatasetEcho_VT0,SemiDatasetEchoValid
...
def training(args):
    ...
    image_size[-1]=args.frame_number
    ...
    train_dataset = SemiDatasetEcho_VT0(data_dir=args.data_dir,
                                   data_csv=args.train_data_csv,
                                   gt_csv=args.train_data_gt,
                                   image_size=image_size,
                                   frame_number=args.frame_number,
                                   label_index=args.label_index,
                                   mode='train',rand_frame=False)
    valid_dataset = SemiDatasetEchoValid(
    ...
    model = TempSeg_Mem_New_ALL('resnet50', None, frame_number=args.frame_number, classes=4,
                        reduction_rate=args.reduction_rate, deep_stem=32,in_channels=2)
    ...
    for epoch in range(epochs):
        ...
        for step, (images, labels,tov_target) in enumerate(train_bar):
            ...
            aug_images, aug_labels = augment_data_batch_frames(aug_images, aug_labels, shift=0, rotate=30,
                                                               scale=0.2,
                                                               flip=False)
            ...
            dice_loss_1 = dice_loss(aug_pred_list[args.label_index], aug_labels.cuda(), softmax=True)
            sup_loss_1 = 0.5 * (F.cross_entropy(aug_pred_list[args.label_index], aug_labels.cuda())
                                + dice_loss_1)
            ...
            dice_loss_2 = dice_loss(masks_split_list[args.label_index], aug_labels.cuda(), softmax=True)

            sup_loss_2 = 0.5 * (F.cross_entropy(masks_split_list[args.label_index], aug_labels.cuda())
                                + dice_loss_2)
