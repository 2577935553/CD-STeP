...
def training(args):
    ...
    model = TempSeg_Mem_New_ALL('resnet50', None, frame_number=args.frame_number, classes=4,
                        reduction_rate=args.reduction_rate, deep_stem=32,in_channels=2,reduc_time=8,use_dste=False)
    ...
    for epoch in range(epochs):
        ...
        for step, (images, labels,tov_target) in enumerate(train_bar):
            ...
            loss_contrast=int(args.TC)*losses_contrast['temporal_contrastive']+int(args.GC)*losses_contrast['global_contrastive']
            # loss_dste=losses_dste['loss_tov']+losses_dste['loss_ad']+losses_dste['loss_rpp']
            loss_dste=losses_dste['loss_temporal_consistency']*0
            ...
            loss_t = sup_loss_2 + unsup_loss_2+tau*loss_dste*0+0.2*tau*loss_contrast
            loss_t.backward()
            optimizer_t.step()
            scheduler_t.step()
