...
def training(args):
    ...
    model = TempSeg_Mem_New_ALL('resnet50', None, frame_number=args.frame_number, classes=4,
                        reduction_rate=args.reduction_rate, deep_stem=32,in_channels=2,reduc_time=8,use_contrast=False,TB=args.TB,SB=args.SB,Gate=args.Gate)
    ...
    for epoch in range(epochs):
        ...
        for step, (images, labels,tov_target) in enumerate(train_bar):
            ...
            loss_dste=losses_dste['loss_temporal_consistency']*int(args.wtloss)
            ...
            loss_t = sup_loss_2 + unsup_loss_2+tau*loss_dste+0.2*tau*loss_contrast*0
            loss_t.backward()
            optimizer_t.step()
            scheduler_t.step()
...
def series_features():
    ...
    model_series = TempSeg_Mem_New_ALL('resnet50', None, frame_number=args.frame_number, classes=4,
                        reduction_rate=args.reduction_rate, deep_stem=32,use_contrast=False)
    if torch.cuda.is_available():
        model_agnostic.cuda()
        model_series.cuda()
    model_agnostic.load_state_dict(torch.load(model_path, map_location=device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wtloss',action='store_false',default=True)
