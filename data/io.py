...
def norm_crop_aug_shuffle_data(image, label, image_size):
    ...
    image = image_utils.crop_image(image, x_centre, y_centre, image_size, constant_values=0)
    label = image_utils.crop_image(label, x_centre, y_centre, image_size, constant_values=0)
    return image, label
...
# def augment_data_batch(image, label, shift=30, rotate=60, scale=0.2, intensity=0.2, frame_number=14, flip=False):
#     aug_image = np.zeros_like(image)  # BxFxHxW
#     aug_image_list = [np.zeros_like(image[:, 1, :, :]) for i in range(frame_number)]
#     image_list = [image[:, i, :, :] for i in range(frame_number)]
#     aug_label = np.zeros_like(label)
#     for i in range(label.shape[0]):
#         tem_image_list = [image[i, :, :] for image in image_list]
#         tem_label = label[i, :, :]
#
...
#         scale_val = 1 + scale * np.random.uniform(-1, 1)
#
#         tem_image_ls = [tem_image * (1 + intensity * np.random.uniform(-1, 1)) for tem_image in tem_image_list]
#
#         # Apply the affine transformation (rotation + scale + shift) to the image
#         row, col = tem_image_ls[0].shape
#         M = cv2.getRotationMatrix2D((row / 2, col / 2), rotate_val, 1.0 / scale_val)
#         M[:, 2] += shift_val
#         M = np.concatenate((M, np.array([[0, 0, 1]])), axis=0)
#
#         aug_image[i, :, :, :] = np.stack([ndimage.interpolation.affine_transform(tem_image, inv(M), order=1)
#                                           for tem_image in tem_image_ls], axis=0)
...
def augment_data_batch_frames(image, label, shift=30, rotate=60, scale=0.2, intensity=0.2, flip=False):
    aug_image = np.zeros_like(image)
    aug_label = np.zeros_like(label)
    frame_nums = image.shape[1]
    for i in range(image.shape[0]):
        tem_image = image[i, :, :, :]
        tem_label = label[i, :, :]
        ...
        # Apply the affine transformation (rotation + scale + shift) to the image
        row, col = tem_label.shape[0:2]
        M = cv2.getRotationMatrix2D((row / 2, col / 2), rotate_val, 1.0 / scale_val)
        M[:, 2] += shift_val
        M = np.concatenate((M, np.array([[0, 0, 1]])), axis=0)
        for j in range(frame_nums):
            aug_image[i, j, :, :] = ndimage.interpolation.affine_transform(tem_image[j], inv(M), order=1)
        aug_label[i, :, :] = ndimage.interpolation.affine_transform(tem_label, inv(M), order=0)

    return aug_image, aug_label


def augment_data_batch_frames_full(image, label, shift=30, rotate=60, scale=0.2, intensity=0.2, flip=False):
    aug_image = np.zeros_like(image)
    aug_label = np.zeros_like(label)
    frame_nums = image.shape[1]
    for i in range(image.shape[0]):
        ...
        # Apply the affine transformation (rotation + scale + shift) to the image
        row, col = tem_label.shape[1:3]
        M = cv2.getRotationMatrix2D((row / 2, col / 2), rotate_val, 1.0 / scale_val)
        M[:, 2] += shift_val
        M = np.concatenate((M, np.array([[0, 0, 1]])), axis=0)
        for j in range(frame_nums):
            aug_image[i, j, :, :] = ndimage.interpolation.affine_transform(tem_image[j], inv(M), order=1)
            aug_label[i, j, :, :] = ndimage.interpolation.affine_transform(tem_label[j], inv(M), order=0)
